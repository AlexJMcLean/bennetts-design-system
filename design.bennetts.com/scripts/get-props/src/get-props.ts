...
export function normalizePath(path: string): string {
  ...
  } else if (normalizedPath.includes("/bds/")) {
...
if (isExecutedThroughCommandLine) {
  globby(["../bds-react/src/**/*.ts", "../bds-react/src/**/*.tsx"]).then(
    (files) => {
      let filesWithoutTests = files.filter(
...
export function getRelevantTypes(
  ...
  function extractTypes(node: Type) {
    ...
    detectedTypeDefinitions?.forEach((name) => {
      if (nonBdsTypes.includes(name)) return;
